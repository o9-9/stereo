...
**Research, signatures, and patch methodology:** o9-9, Cypher, Shaun, Oracle, Crue, Geeko
...
The **Stereo Hub** can download/sync the finder into its cache and launch **Offset Finder (experts)** from Advanced options. In this monorepo, canonical scripts live under **`packages/offset-finder/scripts/`** (the hub may still use a local **`Updates/Offset Finder/`** copy under `packages/system`).
