...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            ...
            echo "  --gui, -g           Launch GUI (auto-detects kdialog/zenity/whiptail/dialog)"
            echo "  --no-gui            Force terminal menu even if GUI toolkits are available"
            echo "  --help, -h          Show this help"
            echo ""
            echo "Examples:"
            echo "  $0                  # GUI if toolkit available; else terminal menu"
            echo "  $0 --gui            # GUI mode (kdialog/zenity/whiptail/dialog)"
            echo "  $0 --no-gui         # Force terminal menu"
...
# ==============================================================================
#  GUI MODE - auto-detects best available toolkit
#  Priority: kdialog (KDE/Qt) -> zenity (GNOME/GTK) -> whiptail -> dialog -> terminal
#  whiptail ships with nearly every Debian/Ubuntu/Fedora out of the box.
# ==============================================================================
GUI_BACKEND=""

detect_gui_backend() {
    # Graphical backends need a display server
...
# --- Abstracted GUI primitives ------------------------------------------------

gui_msgbox() {
    local title="$1" text="$2"
    case "$GUI_BACKEND" in
        kdialog)  kdialog --title "$title" --msgbox "$text" ;;
        zenity)   zenity --info --title="$title" --text="$text" --width=420 --no-wrap 2>/dev/null ;;
        whiptail) whiptail --title "$title" --msgbox "$text" 14 64 ;;
        dialog)   dialog --backtitle "Discord Voice Fixer" --title "$title" --msgbox "$text" 14 64 ;;
    esac
}
...
gui_yesno() {
    ...
    case "$GUI_BACKEND" in
        ...
        zenity)   zenity --question --title="$title" --text="$text" --width=380 --no-wrap 2>/dev/null ;;
        whiptail) whiptail --title "$title" --yesno "$text" 10 60 ;;
        dialog)   dialog --backtitle "Discord Voice Fixer" --title "$title" --yesno "$text" 10 60 ;;
    esac
}
...
gui_menu() {
    ...
    shift 2
    # All backends receive pairs: tag1 label1 tag2 label2 ...
    case "$GUI_BACKEND" in
        kdialog)
        ...
        zenity)
            ...
            zenity --list --title="$title" --text="$prompt" \
                --column="ID" --column="Action" --hide-column=1 \
                --width=500 --height=420 --print-column=1 \
                "${z_args[@]}" 2>/dev/null
            ;;
        whiptail)
            whiptail --title "$title" --menu "$prompt" 20 68 10 "$@" 3>&1 1>&2 2>&3
            ;;
        dialog)
            dialog --stdout --backtitle "Discord Voice Fixer" --title "$title" \
                --menu "$prompt" 20 68 10 "$@"
            ;;
    esac
}
...
gui_textbox() {
    ...
    case "$GUI_BACKEND" in
        kdialog)  kdialog --title "$title" --textbox "$file" 640 480 ;;
        zenity)   zenity --text-info --title="$title" --filename="$file" --width=640 --height=480 2>/dev/null ;;
        whiptail) whiptail --title "$title" --scrolltext --textbox "$file" 22 76 ;;
        dialog)   dialog --backtitle "Discord Voice Fixer" --title "$title" --textbox "$file" 22 76 ;;
    esac
}

# --- Progress bar (backend-specific) -----------------------------------------
# kdialog uses DBus, zenity reads stdin %, whiptail/dialog read stdin %.
# We provide start/update/close for kdialog, and a pipe helper for the rest.
...
gui_gauge_start() {
    ...
    case "$GUI_BACKEND" in
        kdialog)
            _GAUGE_DBUS_REF=$(kdialog --title "$title" --progressbar "$label" 100 2>/dev/null)
            ;;
        zenity)
            local fifo
            fifo=$(mktemp -u)
            mkfifo "$fifo"
            zenity --progress --title="$title" --text="$label" --percentage=0 \
                --width=400 --auto-close --no-cancel < "$fifo" 2>/dev/null &
            _GAUGE_ZENITY_PID=$!
            exec 7>"$fifo"
            _GAUGE_ZENITY_FD=7
            rm -f "$fifo"
        ...
        whiptail|dialog)
            ...
            fifo=$(mktemp -u)
            mkfifo "$fifo"
            if [[ "$GUI_BACKEND" == "whiptail" ]]; then
                whiptail --title "$title" --gauge "$label" 8 60 0 < "$fifo" &
            else
                dialog --backtitle "Discord Voice Fixer" --title "$title" --gauge "$label" 8 60 0 < "$fifo" &
            fi
            _GAUGE_ZENITY_PID=$!
            exec 7>"$fifo"
            _GAUGE_ZENITY_FD=7
...
gui_gauge_update() {
    ...
    case "$GUI_BACKEND" in
        kdialog)
            if [[ -n "$_GAUGE_DBUS_REF" ]]; then
                qdbus $_GAUGE_DBUS_REF Set "" value "$pct" 2>/dev/null || true
                [[ -n "$label" ]] && qdbus $_GAUGE_DBUS_REF setLabelText "$label" 2>/dev/null || true
            fi
            ;;
        zenity)
            if [[ -n "$_GAUGE_ZENITY_FD" ]]; then
                echo "$pct" >&7 || true
                [[ -n "$label" ]] && { echo "# $label" >&7 || true; }
            fi
            ;;
        whiptail|dialog)
            if [[ -n "$_GAUGE_ZENITY_FD" ]]; then
...
gui_gauge_close() {
    case "$GUI_BACKEND" in
        kdialog)
            [[ -n "$_GAUGE_DBUS_REF" ]] && { qdbus $_GAUGE_DBUS_REF close 2>/dev/null || true; }
            _GAUGE_DBUS_REF=""
            ;;
        zenity|whiptail|dialog)
            if [[ -n "$_GAUGE_ZENITY_FD" ]]; then
                exec 7>&- 2>/dev/null || true
                _GAUGE_ZENITY_FD=""
            fi
            [[ -n "$_GAUGE_ZENITY_PID" ]] && { wait "$_GAUGE_ZENITY_PID" 2>/dev/null || true; }
            _GAUGE_ZENITY_PID=""
            ;;
    esac
}
...
run_gui() {
    if ! detect_gui_backend; then
        status "[!] No GUI toolkit found (tried kdialog, zenity, whiptail, dialog)" yellow
        status "    Falling back to terminal menu..." yellow
        run_interactive
        return
    fi

    log_file "INFO" "Starting GUI mode ($GUI_BACKEND) v$SCRIPT_VERSION"
    ...
    if [[ ${#CLIENT_NAMES[@]} -eq 0 ]]; then
        gui_msgbox "Error" "No Discord installations found.\n\nMake sure Discord is installed and has been opened at least once.\nJoin a voice channel to download the voice module, then run this again.\n\nIf you ran with sudo, the script checks the invoking user config."
        exit 1
    fi

    while true; do
        local choice
        choice=$(gui_menu "Discord Voice Fixer v${SCRIPT_VERSION}" \
            "48kHz | 384kbps | True Stereo - Found ${#CLIENT_NAMES[@]} client(s)" \
            "1" "Fix one client" \
            "2" "Fix all clients" \
            "3" "Verify fix status" \
            "4" "Restore from backup" \
            "5" "Diagnostics" \
            "6" "Cleanup invalid backups" \
            "7" "Open backup folder" \
            "8" "Check for script update" \
            "Q" "Quit")

        [[ -z "$choice" ]] && { save_settings; exit 0; }

        case "$choice" in
            1)
                local i menu_args=()
                for i in "${!CLIENT_NAMES[@]}"; do
                    menu_args+=("$i" "${CLIENT_NAMES[$i]} (v${CLIENT_VERSIONS[$i]})")
                done
                local sel
                sel=$(gui_menu "Select client" "Choose client to fix:" "${menu_args[@]}")
                [[ -n "$sel" ]] && gui_fix_single "$sel"
                ;;
            2)
                gui_yesno "Fix all" "Fix all ${#CLIENT_NAMES[@]} client(s)? Discord will be closed." && gui_fix_all
                ;;
            3) gui_verify ;;
            4) gui_restore ;;
            5) gui_diagnostics ;;
            6)
                cleanup_invalid_backups
                gui_msgbox "Cleanup" "Cleanup finished."
                ;;
            7)
                ensure_app_dirs
                if command -v xdg-open &>/dev/null; then
                    xdg-open "$APP_DATA_ROOT" 2>/dev/null
                else
                    gui_msgbox "Backups" "Backup folder:\n$APP_DATA_ROOT"
                fi
                ;;
            8)
                check_script_update || true
                gui_msgbox "Update check" "Check complete. See terminal for details."
                ;;
            Q)
                save_settings
                exit 0
...
gui_fix_single() {
    local idx="$1"
    local name="${CLIENT_NAMES[$idx]}"
    local tmp_dir fix_rc=1

    tmp_dir=$(mktemp -d)
    trap 'rm -rf "$tmp_dir" 2>/dev/null; trap - RETURN' RETURN

    gui_gauge_start "Applying fix" "Downloading voice modules..."

    SILENT_MODE=true download_voice_files "$tmp_dir/VoiceBackup" 2>/dev/null || true
    gui_gauge_update 50 "Closing Discord..."

    kill_discord 2>/dev/null
    sleep 1
    gui_gauge_update 70 "Applying fix to $name..."

    SILENT_MODE=true fix_client "$idx" "$tmp_dir/VoiceBackup" 2>/dev/null
    fix_rc=$?
    gui_gauge_update 100 "Done"
    sleep 0.3
    gui_gauge_close

    remove_old_backups 2>/dev/null || true

    if [[ "$fix_rc" -eq 0 ]]; then
        gui_msgbox "Success" "Fix applied to $name.\n\nRestart Discord to use stereo voice."
    else
        gui_msgbox "Warning" "Fix failed or was cancelled.\nCheck log: $LOG_FILE"
    fi

    if $AUTO_RESTART_DISCORD; then
        gui_yesno "Start Discord" "Start Discord now?" && start_discord
    fi
}

gui_fix_all() {
    local tmp_dir any_fail=0
    tmp_dir=$(mktemp -d)
    trap 'rm -rf "$tmp_dir" 2>/dev/null; trap - RETURN' RETURN

    gui_gauge_start "Fixing all clients" "Downloading voice modules..."

    SILENT_MODE=true download_voice_files "$tmp_dir/VoiceBackup" 2>/dev/null || true
    gui_gauge_update 30 "Closing Discord..."

    kill_discord 2>/dev/null
    sleep 1
    gui_gauge_update 40 "Applying fixes..."

    local n=${#CLIENT_NAMES[@]} i
    for i in "${!CLIENT_NAMES[@]}"; do
        local pct=$(( 40 + ( 50 * (i + 1) ) / n ))
        gui_gauge_update "$pct" "Fixing ${CLIENT_NAMES[$i]}..."
        SILENT_MODE=true fix_client "$i" "$tmp_dir/VoiceBackup" 2>/dev/null || any_fail=1
    done
    ...
    if [[ "$any_fail" -eq 0 ]]; then
        gui_msgbox "Complete" "Fix applied to all clients.\n\nRestart Discord to use stereo voice."
    else
        gui_msgbox "Warning" "One or more fixes failed.\nCheck log: $LOG_FILE"
    fi
...
gui_verify() {
    ...
    for i in "${!CLIENT_NAMES[@]}"; do
        ...
        rmsg="${rmsg%%|*}"
        case "$rstatus" in
            FIXED)    msg+="${CLIENT_NAMES[$i]}: [OK] Stereo fix active\n" ;;
            NOTFIXED) msg+="${CLIENT_NAMES[$i]}: [--] Original mono (run fix)\n" ;;
            UNKNOWN)  msg+="${CLIENT_NAMES[$i]}: [??] Unknown\n" ;;
            ERROR)    msg+="${CLIENT_NAMES[$i]}: [!!] $rmsg\n" ;;
        esac
    done
    gui_msgbox "Verify fix status" "${msg:-No clients}"
}
...
gui_restore() {
    ...
    backup_list=$(list_backups)
    if [[ -z "$backup_list" ]]; then
        gui_msgbox "Restore" "No valid backups found.\n\nRun the fix at least once to create a backup."
        return
    fi

    local backup_paths=() backup_originals=() backup_tags=() i=0
    local menu_args=()
    while IFS='|' read -r btype bpath bcn bav bdate; do
        backup_paths+=("$bpath")
        [[ "$btype" == "ORIGINAL" ]] && backup_originals+=("true") || backup_originals+=("false")
        backup_tags+=("$i")
        if [[ "$btype" == "ORIGINAL" ]]; then
            menu_args+=("$i" "[ORIGINAL] $bcn v$bav - $bdate")
        else
            menu_args+=("$i" "$bcn v$bav - $bdate")
        fi
        (( i++ )) || true
    done <<< "$backup_list"

    local sel
    sel=$(gui_menu "Restore from backup" "Select backup to restore:" "${menu_args[@]}")
    [[ -z "$sel" ]] && return
    local idx="$sel"

    local target_args=() k
    for k in "${!CLIENT_NAMES[@]}"; do
        target_args+=("$k" "${CLIENT_NAMES[$k]} (v${CLIENT_VERSIONS[$k]})")
    done
    local target_sel
    target_sel=$(gui_menu "Restore to client" "Select target client:" "${target_args[@]}")
    [[ -z "$target_sel" ]] && return
    local tidx="$target_sel"

    if [[ "${backup_originals[$idx]}" == "true" ]]; then
        gui_yesno "Confirm" "This will revert to ORIGINAL mono audio.\nYou will lose the stereo fix for this client.\n\nContinue?" || return
    fi

    gui_gauge_start "Restore" "Closing Discord..."

    kill_discord 2>/dev/null
    sleep 1
    gui_gauge_update 50 "Restoring backup..."

    local restore_rc=1
    SILENT_MODE=true restore_from_backup "${backup_paths[$idx]}" "${CLIENT_VOICE_PATHS[$tidx]}" "${backup_originals[$idx]}" 2>/dev/null
    restore_rc=$?
    ...
    if [[ "$restore_rc" -eq 0 ]]; then
        gui_msgbox "Restore" "Restore complete.\n\nRestart Discord to apply changes."
    else
        gui_msgbox "Restore" "Restore failed.\nCheck log: $LOG_FILE"
    fi
...
gui_diagnostics() {
    local tmp_out
    tmp_out=$(mktemp)
    trap 'rm -f "$tmp_out" 2>/dev/null; trap - RETURN' RETURN

    {
        ...
        for dep in curl jq md5sum; do
            if command -v "$dep" &>/dev/null; then
                ...
                echo "  [X]  $dep - NOT FOUND"
            fi
        done
        echo ""
        echo "Discord installations:"
        find_discord_clients
        if [[ ${#CLIENT_NAMES[@]} -eq 0 ]]; then
            ...
            for i in "${!CLIENT_NAMES[@]}"; do
                echo ""
                echo "  ${CLIENT_NAMES[$i]} (v${CLIENT_VERSIONS[$i]})"
                echo "    Voice path: ${CLIENT_VOICE_PATHS[$i]}"
                local result
                result=$(verify_fix "${CLIENT_VOICE_PATHS[$i]}" "${CLIENT_NAMES[$i]}")
                local rstatus="${result%%|*}"
                case "$rstatus" in
                    FIXED)    echo "    Status: STEREO ACTIVE" ;;
                    NOTFIXED) echo "    Status: Original mono" ;;
                    UNKNOWN)  echo "    Status: Unknown" ;;
                    ERROR)    echo "    Status: ${result#*|}" ;;
                esac
            done
        fi
    } > "$tmp_out" 2>&1

    gui_textbox "System diagnostics" "$tmp_out"
}
