...
const useLegacyAudioDevice = appSettings ? appSettings.getSync('useLegacyAudioDevice') : true;
     ...
     setTransportOptions: (options) => {
      ...
      if (options.audioEncoder) {
        options.audioEncoder.params = {
          ...
          "sprop-stereo": 1, // Ensure stereo is used
          maxaveragebitrate: 400000, // Maximum average bitrate for highest quality
          useinbandfec: 0, // Disable in-band forward error correction
          usedtx: 0, // Disable discontinuous transmission (DTX)
          cbr: 1, // Use constant bitrate
          maxplaybackrate: 96000 // Maximum playback rate
        };
        options.audioEncoder.channels = 2; // Stereo channels
        options.audioEncoder.freq = 96000; // Standard frequency for high-quality audio
        options.audioEncoder.rate = 400000; // High bitrate for lossless quality
        options.audioEncoder.pacsize = 300; // Packet size (adjust as necessary)
        options.audioEncoder.bits_per_sample = 24; // Sample size in bits (adjust as necessary)
      }

      // Disable all microphone processing features
      options.disable_agc = true; // Disable automatic gain control
      options.disable_noise_suppression = true; // Disable noise suppression
      options.disable_echo_cancellation = true; // Disable echo cancellation

      if (options.encodingVoiceBitRate) {
        options.encodingVoiceBitRate = 400000; // Ensure a high bitrate for voice encoding
      }
