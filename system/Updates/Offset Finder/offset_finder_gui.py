...
def _finder_script_search_dirs() -> list[Path]:
    """Prefer OFFSET FINDER/scripts (monorepo: packages/offset-finder/scripts); also Stereo Hub cache."""
    dirs = [SCRIPT_DIR]
...
class OffsetFinderGUI:
    ...
    def _run_finder(self):
        ...
        if self.finder_module is None:
            ...
            self._append_output(
                "  Put discord_voice_node_offset_finder_v5.py in OFFSET FINDER/scripts or sync it via Stereo Hub / manifest.\n",
                "info",
            )
            return
