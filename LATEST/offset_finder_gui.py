#!/usr/bin/env python3
# Offset Finder GUI — loads finder script from same dir. Dark theme.
...
class OffsetFinderGUI:
    def __init__(self, root):
        ...
        # Stored results for formatted save
        self._last_results = None
        self._last_bin_info = None
        self._last_file_path = None
        self._last_file_size = None
        self._last_adj = 0
        self._last_tiers_used = {}
    ...
    def _build_ui(self):
        ...
        self.save_btn = self._make_button(btn_frame, "Save Offsets", GRAY_BTN, GRAY_HOVER,
                                          self._save_results)
    ...
    def _save_results(self):
        if not self._last_results:
            self.status_var.set("Nothing to save — run the finder first")
            return

        mod = self.finder_module
        results = self._last_results
        bin_info = self._last_bin_info
        fpath = self._last_file_path
        fsize = self._last_file_size
        adj = self._last_adj
        tiers = self._last_tiers_used
        fmt = bin_info.get('format', 'unknown') if bin_info else 'unknown'

        path = filedialog.asksaveasfilename(
            title="Save Patcher Offsets",
        ...
        # ── Header ──
        platform_label = {"pe": "Windows (PE)", "elf": "Linux (ELF)",
                          "macho": "macOS (Mach-O)"}.get(fmt, fmt.upper())
        lines.append(sep)
        lines.append(f"  Discord Voice Node Offset Finder v{getattr(mod, 'VERSION', '?')}")
        lines.append(f"  Platform: {platform_label} | Offsets: {len(results)}/18")
        if fpath:
            lines.append(f"  File: {fpath}")
        lines.append(sep)
        lines.append("")

        # ── Windows PowerShell section (always included — universal reference format) ──
        if fmt == 'pe':
            lines.append(sep)
            lines.append("  WINDOWS PATCHER  (Discord_voice_node_patcher.ps1)")
            lines.append("  Copy the Offsets block below into $Script:Config")
            lines.append(sep)
        else:
            lines.append(sep)
            lines.append("  POWERSHELL FORMAT  (reference)")
            lines.append("  This is the offset finder's canonical output format")
            lines.append(sep)
        lines.append("")
        try:
            ps = mod.format_powershell_config(results, bin_info=bin_info,
                                              file_path=fpath, file_size=fsize)
            lines.append(ps)
        except Exception as e:
            lines.append(f"    # Error generating PS config: {e}")
        lines.append("")
        lines.append("")

        # ── Bash section (only for the platform that was scanned) ──
        if fmt == 'elf':
            lines.append(sep)
            lines.append("  LINUX PATCHER  (discord_voice_patcher_linux.sh)")
            lines.append("  Copy the declare block below, replacing the existing OFFSETS")
            lines.append(sep)
            lines.append("")
            try:
                bash = mod.format_bash_offsets(results, bin_info=bin_info,
                                              file_path=fpath, file_size=fsize)
                lines.append(bash)
            except Exception as e:
                lines.append(f"# Error generating bash config: {e}")
            lines.append("")
            lines.append("")
        elif fmt == 'macho':
            lines.append(sep)
            lines.append("  macOS PATCHER  (discord_voice_patcher_macos.sh)")
            lines.append("  Copy the block below into the patcher — replace OFFSET_* and ARM64_* section")
            lines.append(sep)
            lines.append("")
            try:
                stereo = bin_info.get("stereo_patches") or []
                copyblock = mod.format_macos_patcher_copyblock(results, adj, stereo)
                lines.append(copyblock)
            except Exception as e:
        ...
        lines.append("")
        try:
            j = mod.format_json(results, bin_info, fpath, fsize, adj, tiers)
            lines.append(j)
        except Exception as e:
            lines.append(f"// Error generating JSON: {e}")
        lines.append("")
    ...
    def _load_finder(self):
        ...
        for f in sorted(SCRIPT_DIR.glob("*.py"), reverse=True):
            ...
            try:
                ...
                if ("SIGNATURES" in text and "VERSION" in text) or \
                   ("discover_offsets" in text and "format_macos_arm64_block" in text):
                    finder_path = f
                    break
            except Exception:
                continue
    ...
    def _run_finder_thread(self, path):
        ...
        try:
            ...
            # macOS: fill stereo_patches so Save outputs patcher-ready block
            if fmt == "macho" and hasattr(mod, "find_macos_stereo_patches"):
                try:
                    sp = mod.find_macos_stereo_patches(data)
                    if sp:
                        bin_info["stereo_patches"] = sp
                except Exception:
                    pass

            # Store for formatted save
            self._last_results = results
            self._last_bin_info = bin_info
            self._last_file_path = path
            self._last_file_size = len(data)
            self._last_adj = adj
            self._last_tiers_used = tiers_used
            ...
            if found == total:
                self._append_output_safe(
                    f"  ✓ ALL {found} OFFSETS FOUND SUCCESSFULLY\n", "success")
            else:
                self._append_output_safe(
                    f"  Found {found}/{total} offsets ({total - found} missing)\n", "warn")
                if errors:
                    for e in errors:
            ...
            # macOS: show patcher copy block (same as CLI)
            if fmt == "macho" and bin_info.get("stereo_patches") and hasattr(mod, "format_macos_patcher_copyblock"):
                try:
                    cb = mod.format_macos_patcher_copyblock(results, adj, bin_info["stereo_patches"])
                    self._append_output_safe("\n--- BEGIN COPY (paste into discord_voice_patcher_macos.sh) ---\n", "header")
                    self._append_output_safe(cb + "\n", None)
            ...
            self._set_status_safe(
                f"Done — {found}/{total} offsets | {datetime.now().strftime('%H:%M:%S')}")
