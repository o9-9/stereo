...
$UPDATE_URL = "https://raw.githubusercontent.com/o9-9/stereo/main/installer.ps1"
$VOICE_BACKUP_API = "https://api.github.com/repos/o9-9/stereo/contents/packages%2Fvoice-backup%2FDiscord%20Voice%20Backup"
$SETTINGS_JSON_URL = "https://raw.githubusercontent.com/o9-9/stereo/main/packages/voice-backup/settings.json"
$DISCORD_SETUP_URL = "https://discord.com/api/downloads/systems/app/installers/latest?channel=stable&platform=win&arch=x64"
...
function Stop-DiscordProcesses { param([string[]]$ProcessNames)
    ...
    $finalRemaining = Get-Process -Name $toKill -ErrorAction SilentlyContinue
    $finalUpdatePids = & $getDiscordUpdatePids
    if ($finalRemaining -or $finalUpdatePids.Count -gt 0) { return $false }
    return $true
}
...
function Get-PathFromShortcuts { param([string]$ShortcutName)
    ...
    $result = $null
    try {
        foreach ($lf in $scs) { try { $sc = $ws.CreateShortcut($lf.FullName); if (Test-Path $sc.TargetPath) { $result = (Split-Path (Split-Path $sc.TargetPath -Parent) -Parent); break } } catch { } }
    } finally {
...
function Reinstall-DiscordClient {
    ...
    try {
        ...
        if ($clientName -match "Canary") {
            ...
            $setupUrl = "https://discord.com/api/downloads/systems/app/installers/latest?channel=canary&platform=win&arch=x64"
        } elseif ($clientName -match "PTB") {
            $discordVariant = "ptb"
            $setupUrl = "https://discord.com/api/downloads/systems/app/installers/latest?channel=ptb&platform=win&arch=x64"
        } elseif ($clientName -match "Development") {
            $discordVariant = "development"
            $setupUrl = "https://discord.com/api/downloads/systems/app/installers/latest?channel=development&platform=win&arch=x64"
        }
...
function Save-StateData { param($State); $State | ConvertTo-Json -Depth 5 | Out-File $STATE_FILE -Force }
...
function Apply-ScriptUpdate { param([string]$UpdatedScriptPath, [string]$CurrentScriptPath)
    ...
    $bc = "@echo off`ntimeout /t 2 /nobreak >nul`nif not exist `"$UpdatedScriptPath`" exit /b 1`ncopy /Y `"$UpdatedScriptPath`" `"$CurrentScriptPath`" >nul`ntimeout /t 1 /nobreak >nul`nstart `"`" powershell.exe -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -WindowStyle Hidden -File `"$CurrentScriptPath`"`ndel `"$UpdatedScriptPath`" >nul 2>&1`n(goto) 2>nul & del `"%~f0`""
    ...
    if ($corruptedClients.Count -gt 0 -and $ic.Count -eq 0) {
        ...
        foreach ($corrupt in $corruptedClients) {
            ...
            try {
                ...
                if (Test-Path $updateExe) { Remove-Item $updateExe -Force -ErrorAction SilentlyContinue }
                $setupUrl = $DISCORD_SETUP_URL
                if ($corrupt.Client.Name -match "Canary") { $setupUrl = "https://discord.com/api/downloads/systems/app/installers/latest?channel=canary&platform=win&arch=x64" }
                elseif ($corrupt.Client.Name -match "PTB") { $setupUrl = "https://discord.com/api/downloads/systems/app/installers/latest?channel=ptb&platform=win&arch=x64" }
                elseif ($corrupt.Client.Name -match "Development") { $setupUrl = "https://discord.com/api/downloads/systems/app/installers/latest?channel=development&platform=win&arch=x64" }
...
$clientCombo.FlatStyle = "Flat"; $clientCombo.Font = $Fonts.Console
...
$btnStart.Add_Click({
    ...
    try {
        ...
        if ($diag.Error) {
            ...
            if ($diag.Error) { Write-Log "Aborting fix after reinstall attempt - diag error persists: $($diag.Error)" "ERROR"; return }
        }
